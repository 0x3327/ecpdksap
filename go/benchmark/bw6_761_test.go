...
func Benchmark_BW6_761(b *testing.B) {

	// _Benchmark_BW6_761(b, 5000, 10)
	// _Benchmark_BW6_761(b, 20_000, 10)
	// _Benchmark_BW6_761(b, 40_000, 10)
	_Benchmark_BW6_761(b, 80_000, 10)
	// _Benchmark_BW6_761(b, 100_000, 10)
}

func _Benchmark_BW6_761(b *testing.B, sampleSize int, nRepetitions int) {

	fmt.Println("Benchmark_BW6_761 ::: sampleSize:", sampleSize, "nRepetitions:", nRepetitions)
	fmt.Println()
	...
	for i := 0; i < nRepetitions; i++ {
		...
		_, v_asBigInt, V, _ := _BW6_761_GenerateG1KeyPair()
		...
		for j := 0; j < sampleSize; j++ {
			...
			_, rj_asBigInt, Rj, Rj_asAff := _BW6_761_GenerateG1KeyPair()
		...
		_, _, _, K2_BW6_761_asAff := _BW6_761_GenerateG2KeyPair()
		K2_BW6_761_asAffArr := []EC.G2Affine{K2_BW6_761_asAff}
		...
		for _, Rsi_asArray := range RsAff_asArr {
			...
			pairingResult, _ := EC.Pair(Rsi_asArray, K2_BW6_761_asAffArr)
		...
		viewTagsSingleByte[len(viewTags)-1] = _BW6_761_G1AffPointToViewTagByte1(&rV_asAff)

		b.ResetTimer()

		for i, Rsi_asArray := range RsAff_asArr {

			if _BW6_761_G1AffPointToViewTagByte1(vR_asAff.FromJacobian(vR.ScalarMultiplication(&Rs[i], &v_asBigInt))) != viewTagsSingleByte[i] {
				continue
			}
		...
		viewTags[len(viewTags)-1] = _BW6_761_G1AffPointToViewTagByte2(&rV_asAff)

		b.ResetTimer()

		for i, Rsi_asArray := range RsAff_asArr {

			if _BW6_761_G1AffPointToViewTagByte2(vR_asAff.FromJacobian(vR.ScalarMultiplication(&Rs[i], &v_asBigInt))) != viewTags[i] {
				continue
			}
		...
		viewTagsSingleByte[len(viewTags)-1] = _BW6_761_G1AffPointXCoordToViewTagByte1(&rV_asAff)

		b.ResetTimer()

		for i, RsiAff_asArray := range RsAff_asArr {

			if _BW6_761_G1AffPointXCoordToViewTagByte1(vR_asAff.FromJacobian(vR.ScalarMultiplication(&Rs[i], &v_asBigInt))) != viewTagsSingleByte[i] {
				continue
			}

			pairingResult, _ := EC.Pair(RsiAff_asArray, K2_BW6_761_asAffArr)
		...
		for _, Rsi_asJac := range Rs {
			...
			hash_asBigInt := _BW6_761_HashG1AffPoint(&vR_asAff)
		...
		for i, Rsi_asJac := range Rs {
			...
			if _BW6_761_G1AffPointToViewTagByte1(vR_asAff.FromJacobian(&vR)) != viewTagsSingleByte[i] {
				continue
			}
		...
		for i, Rsi_asJac := range Rs {
			...
			if _BW6_761_G1AffPointToViewTagByte2(vR_asAff.FromJacobian(&vR)) != viewTags[i] {
				continue
			}
		...
		for i, Rsi_asJac := range Rs {
			...
			if _BW6_761_G1AffPointXCoordToViewTagByte1(&vR_asAff) != viewTagsSingleByte[i] {
				continue
			}
		...
		for _, Rsi_asJac := range Rs {
			...
			if _BW6_761_G1AffPointToViewTagByte1(&vR_asAff) != viewTagsSingleByte[i] {
				continue
			}
		...
		for _, Rsi_asJac := range Rs {
			...
			if _BW6_761_G1AffPointToViewTagByte2(&vR_asAff) != viewTags[i] {
				continue
			}
...
func _BW6_761_GenerateG2KeyPair() (privKey EC_fr.Element, privKey_asBigIng big.Int, pubKey EC.G2Jac, pubKeyAff EC.G2Affine) {
...
func _BW6_761_G1AffPointToViewTag(pt *EC.G1Affine, len uint) (viewTag string) {
...
func _BW6_761_G1AffPointXCoordToViewTag(pt *EC.G1Affine, len uint) (viewTag string) {
